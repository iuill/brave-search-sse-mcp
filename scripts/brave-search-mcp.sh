...
DEFAULT_PORT_FROM_ENV="3005" # フォールバック値
...
_check_env_file() {
  ...
  if [ ! -f "$env_path" ]; then
    ...
    echo "BRAVE_API_KEY 環境変数が設定されていることを確認してください。" >&2
    echo "例: .env ファイルを作成し、以下の内容を設定してください:" >&2
    echo "BRAVE_API_KEY=your_api_key_here" >&2
    echo "PORT=3005" >&2
    return 1
  fi

  # BRAVE_API_KEYが設定されているか確認
  if ! grep -q "BRAVE_API_KEY=" "$env_path"; then
    echo "警告: .env ファイルに BRAVE_API_KEY が設定されていません。" >&2
    echo "Brave Search API を使用するには API キーが必要です。" >&2
    return 1
  fi
